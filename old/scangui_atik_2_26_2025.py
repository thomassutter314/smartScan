...
class ScanApp():
    ...
    def endScan(self, finalImage, finalScanNumber, makeDataPlots = True):
        ...
        self.root.quit()
         ...
         # Totally destroy this scan gui so we can move into the setup gui
        self.root.destroy()
        
        # start the setup app again
        setupapp = SetupApp(camera=self.camera,exposure=self.exposure,ds=self.ds,hwp=self.hwp, rm=self.rm)
