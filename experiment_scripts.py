...
def heater_and_ds_scan(dsPosList, hwpPosList, exposure, gain, scanDir, dsWait = 0.4, hwpWait = 5, loopsPerHwpPos = 3):
    scanLive = True
    # Initialize camera
    camera = lab_instruments.Camera()
    time.sleep(1)
    ...
    # Wait here for user to begin scan
    print('________________________________')
    beginCode = input('PRESS ENTER TO START HEATERLINE SCAN \n')
    ...
    while scanLive:
        ...
        for f in range(len(hwpPosList)):
            ...
            # move to next hwp pos
            hwp.moveAbsolute(hwpPosList[fi])
            readHwpPos = hwp.getPos() # Read the half wave plate value back to compare to the send value
            
            print(f'fi = {fi}, hwpPos = {hwpPosList[fi]}')
            print(f'Waiting for {hwpWait} seconds at this hwp position so that temp is steady')
            print(f'hwp read back position, set position = {readHwpPos, hwpPosList[fi]}')
            time.sleep(hwpWait)
            for li in range(loopsPerHwpPos):
                for pi in range(len(dsPosList)):
                    # check for user attempting to exit
...
if __name__ == '__main__':
    ...
    dsPosList = [55.8, 55.4, 55.0]
    hwpPosList = np.linspace(53, 78, 30, endpoint = True)
    heater_and_ds_scan(dsPosList = dsPosList, hwpPosList = hwpPosList, exposure = 16, gain = 30, loopsPerHwpPos = 5, scanDir = r'C:\Users\Kogar\Documents\electron_beam_photos\scans\2024_8_7_overnight_heater_3_point_scan_extravaganaza _halapalooza')
